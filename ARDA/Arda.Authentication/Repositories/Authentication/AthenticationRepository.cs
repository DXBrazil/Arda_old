...
namespace Arda.Authentication.Repositories.Authentication
{
    public class AthenticationRepository : IAuthentication
    {
        ...
        {
            User user;
            //string incomingPasswordHashed = PasswordStorage.CreateHash(incomingPassword);
            bool ComparisonResult; 

            // Recover user based on email and password.
            try
            {
                user = _context.Users.FirstOrDefault(u => u.Email == email);
                ComparisonResult = PasswordStorage.VerifyPassword(incomingPassword, user.Password);
            }
            catch (Exception)
            {
                return null;
            ...
            // Verify if returned user is valid or is null.
            if(user == null)
            {
                return null;
            }
